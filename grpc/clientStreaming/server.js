const PROTO_PATH = './protos/client.proto';  //proto타입의 경로
...
// protoLoader 모듈을 통해 만든 package Definition을 통해, serviceProto를 등록하고, proto파일에 지정된 package명(여기서는 client_service)으로 불러옴
...
const requestFunc = (call, callback) => {
    ...
    const value = parseInt(call.request.message.split('message')[1]);
...
const main = () => {
    ...
    //각각의 인자는 serviceProto의 서비스 이름(프로토 파일 내에는 ClientService로 선언)
    //proto에 정의된 Request함수는 위에서 정의한 requestFunc를 사용 = 이후 해당 서비스로 요청이 온다면 requestFunc를 통해 client에게 응답한다.
    server.addService(serviceProto.ClientService.service, {
